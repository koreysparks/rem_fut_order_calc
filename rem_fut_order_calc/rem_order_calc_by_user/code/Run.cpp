...
bool loadParam()
{
	string paramName = "rem_order_calc_by_user.xml";
	xml_document doc;
...
void* calcFun(void* lParam)
{
	...
	CDB db(dbP->m_host, dbP->m_port, dbP->m_dbName, dbP->m_user, dbP->m_pwd, &log);
	CCalcTrade calc(&db);
	calc.Run();
